 ...
 *
 *   - 数据:与上游同源 —— 经 __DSH_WEB_URL__(扩展代理)调 session.list RPC(与上游
 *     workspace store 同一 wire 契约),不做本地缓存/硬编码;
 ...
 *     返回即回到会话页打开前的聊天视图,不是回到侧边栏)。
 *
 * 组件边界:SessionManagementView → SessionPageHeader(Back + BrandWordmark + New) +
 * SessionList → SessionListItem(整行可点,直接跳转)。
 */
...
/** DeepSeek Harness 品牌 wordmark(与上游 ui-primitives BrandWordmark 同源提取;ink 用 currentColor)。 */
...
/* ---- 数据模型:与上游 workspace store 同 wire(session.list RPC) ---- */
...
function useSessions(): { items: SessionItem[]; loading: boolean; error: string | null; reload: () => void } {
  ...
  useEffect(() => {
    ...
    rpc('session.list', {})
      .then((body) => {
        if (cancelled) return;
        const value = (body as { result?: { value?: { items?: unknown[] } } }).result?.value;
        const raw = Array.isArray(value?.items) ? value.items : [];
        const list = raw
          .map((it): SessionItem | undefined => {
            const item = it as {
              sessionId?: string;
              updatedAt?: number;
              blank?: boolean;
          ...
          .filter((it): it is SessionItem => it !== undefined);
...
function SessionListItem({
  ...
  const cls = 'dsh-session-item' + (current ? ' dsh-session-item-current' : '');
  return (
    <li>
      <button type="button" className={cls} onClick={() => onOpen(item.sessionId)}>
        <span className="dsh-session-title">{item.title !== '' ? item.title : str('新会话', 'New session')}</span>
        {item.running && <span className="dsh-session-running" aria-hidden="true" />}
      </button>
    </li>
  );
}
...
function SessionList({
  ...
  if (loading) return <div className="dsh-session-state">{str('加载中…', 'Loading…')}</div>;
  if (error !== null) {
    return (
      <div className="dsh-session-state">
        <div>{str('会话加载失败', 'Failed to load sessions')}</div>
        <div className="dsh-session-error">{error}</div>
        <button type="button" className="dsh-session-retry" onClick={onRetry}>
          {str('重试', 'Retry')}
        </button>
      </div>
    );
  }
  if (items.length === 0) return <div className="dsh-session-state">{str('暂无会话', 'No sessions')}</div>;
  return (
    <ul className="dsh-session-list">
      {items.map((item) => (
        <SessionListItem key={item.sessionId} item={item} current={item.sessionId === currentId} onOpen={onOpen} />
      ))}
    </ul>
  );
}
...
export function SessionManagementView(): React.JSX.Element {
  const { items, loading, error, reload } = useSessions();
  ...
  return (
    <div className="dsh-session-page">
      ...
      <main className="dsh-session-body">
        <h1 className="dsh-session-section">{str('会话', 'Sessions')}</h1>
        <SessionList items={items} currentId={currentId} loading={loading} error={error} onOpen={openSession} onRetry={reload} />
      </main>
    </div>
  );
}
