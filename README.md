...
**Route A（源码构建）已落地：UI = dsh 上游原生 React 组件，从锁定 rev 的 vendor 源码构建装配（Phase 5–6 ✅，Phase 7–8 进行中，Phase 9 UI/UX 定制 ✅，Phase 10 已实施待冒烟）。** 开发契约（红线、规范、协议、坑）见 [AGENTS.md](AGENTS.md)；完整任务书与进度见 [TASK.md](TASK.md)；协议缺口与降级见 [docs/gaps.md](docs/gaps.md)；手动测试清单见 [docs/manual-tests/](docs/manual-tests/)。
